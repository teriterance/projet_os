...
int main(int argc, char * argv[]){
    ...
    while (1)
    {
        ...
        if (choix == OUI){
            ...
            printf("bonjour`");
            printf("ouverteure du fichier %s", file_loc);
            ...
            if (choix ==  NON){
                ...
                printf("bonjour`");
                printf("ouverteure du fichier %s", file_loc);
                FILE *file_out = fopen(file_loc, "w");
                if(file_out !=  NULL){
                    short_m n = OUI;
                    send(socket_service, &n , sizeof(short_m), 0);
                }
                ...
                long taille_fichier;
                recv(socket_service, &taille_fichier, sizeof(long), 0);

                //on recois notre message
                char contenu_fichier[taille_fichier];
                recv(socket_service, contenu_fichier, taille_fichier*sizeof(char), 0);

                //on ouvre le fichier en question (en ecriture)
                FILE *file_int = fopen(contenu_fichier, "w");
